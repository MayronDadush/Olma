...
- **A tag from somebody the gateway would have dropped is answered: a pause their next word would end is ENDED by it, and a roster row gets one fixed line** (`syncSenderGate` → `pause.endsOnWrite`/`resumeOnWrite`; `reason: 'pending_sender'` is the only addressed claim)
