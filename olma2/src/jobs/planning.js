...
function buildBrief({ user, tasks, reminders, events, facts, now }) {
  ...
  return [
    ...
    `  "bullets": ["0 to ${MAX_BULLETS} short Hebrew lines — as few as the day earns"],`,
    ...
    'reminder to send greetings, never that it greets anyone itself.',
    '',
    `${MAX_BULLETS} is a CEILING, not a target. Write only the lines this day`,
    'actually earns and then stop: six of the first seven plans built here came',
    'back with exactly five, which is what padding looks like from the outside.',
    'A line that would be true for anyone on any day is worse than no line at',
    'all, because the assistant reads every one of them and can spend its opening',
    'on the weakest.',
    '',
    'At most ONE line may be a suggestion, and only when it follows from something',
    'specific above — a named task, a real gap in the week, a date that is actually',
    'coming. If nothing in the data supports one, leave it out. A quiet day is a',
    'normal outcome: say so briefly rather than inventing urgency.',
    '',
