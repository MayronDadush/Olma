...
const TOOLS = [
  ...
  tool('add_task', 'Add one task. A dictated shopping run ("לקנות חלב, קוטג׳ וגבינה צהובה") is recognised and filed for you: the reply comes back with shoppingList:true, the list task, and items — say what went on the list, not that you created a task. merged:true means it joined the run already open rather than starting a second; alreadyOnList names what was there; dueAtIgnored means a date they gave was NOT applied to a list that already existed — offer it rather than assume it. Use parent_task_id to add a subtask to a project (one level). A due_at gets its REMINDER AUTOMATICALLY — an hour before a task with a time, 08:00 that morning for a whole-day one — and the reply carries the reminders created. Do not ask permission for it and never call set_task_reminder just to add the obvious one; simply say when you will remind them. Use set_task_reminder only when they ask for a DIFFERENT moment or a repeat (it replaces the automatic one). If due_at is given it MUST carry a UTC offset (2026-08-20T09:00:00+03:00) — a bare local time is rejected; convert from the user\'s own stated local time using their timezone (USER.md), never write their local digits with a bare Z.',
  ...
  tool('remember_fact', 'Store a durable fact about this person (still matters in a month). Write it in THIRD PERSON, from your side: "מאיה היא אשתו", never "אשתי" — facts are injected into USER.md every turn as things YOU know, so their own words copied through read as statements about you. First person is refused. NOT a task (add_task), NOT a phone/who-knows-whom (connections), NOT how they like you to work (remember_preference), NOT Olma\'s own state (calendar/digest/connection status — the card already carries it). A constraint about ONE arrangement ("לא נוח לי בשבת הקרובה") belongs to that meeting via record_meeting_constraint; store it here only if they generalise it ("אני אף פעם לא נפגשת בשבת"), and a standing availability rule is remember_preference key availability. importance: 1 ordinary / 2 important / 3 core-only-if-always-relevant. expires_at is REQUIRED when the fact names a date or a moving day ("היום", "מחר", "29.8") — it is refused without one.',
