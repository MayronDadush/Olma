...
const FLAG_SPECS = [
  ...
  { key: 'google_connect_phones', label: 'חיבור גוגל (יומן ואנשי קשר) — מי מורשה', type: 'text',
    help: 'ריק = סגור לכולם חוץ מאדמין; "all" = פתוח לכולם; או מספרי טלפון ב-E.164 מופרדים בפסיק. חוסם רק יצירת חיבור חדש — מי שכבר מחובר ממשיך לעבוד רגיל, ושום דבר לא מנותק. כשזה סגור עולמה גם לא מציעה את זה מיוזמתה. להשאיר סגור עד שהאימות של האפליקציה מול גוגל יסתיים, כדי שאף אחד לא ינחת על מסך "האפליקציה לא מאומתת".' },
  { key: 'jev_shadow_twins', label: 'Jev במצב צל — זיהוי משימות כפולות', type: 'bool',
    help: 'פתוח — כל עשר דקות, על משימות חדשות מהיממה האחרונה, Jev נשאל אם המשימה כבר קיימת ברשימה הפתוחה, והתשובה נרשמת ליד התשובה של הקוד. שום דבר לא משתנה למשתמשים: לא מיזוג, לא שאלה, לא הודעה. מה שיוצא החוצה: כותרות המשימות והרשימה הפתוחה לצידן, ל-OpenRouter. לא משתמש הבדיקה הלילי. סגור — מפסיק מהסבב הבא.' },
  ...
  { key: 'dashboard_call_phones', label: 'שיחת טלפון מהדף האישי — מי מורשה', type: 'text',
    help: 'ריק = סגור לכולם חוץ מאדמין; "all" = פתוח לכולם; או מספרי טלפון ב-E.164 מופרדים בפסיק. בכל מקרה — שתי שיחות לכל היותר, אי פעם, לכל משתמש (מכסה חד-פעמית, לא מתחדשת), עד 120 שניות לשיחה. גשר הקול הוא עדיין זה שקובע בפועל למי לחייג — פתיחה כאן לא מחייגת למספר שהגשר לא משרת.' },
  ...
  { key: 'group_open_without_everyone', label: 'קבוצה נפתחת גם אם לא כולם נרשמו', type: 'bool',
    help: 'פתוח — קבוצה נפתחת, מקבלת סוכן ואפשר להתחיל בה תיאום פגישות ברגע ששני אנשים לפחות כתבו לעולמה בפרטי, גם אם חברים אחרים בקבוצה עוד לא. מי שלא כתב נשאר רשום כחסר, לא נספר בתיאום ולא מקבל שום הודעה. סגור — הכל או כלום: הקבוצה נשארת נעולה עד שכל מי שבה כתב לעולמה, והדבר היחיד שהיא אומרת שם הוא "עוד לא שלחו לי: …". שים לב: בקבוצה שנפתחה בלי כולם השורה "יש! כולם כאן" לא נאמרת — היא לא נכונה — והקבוצה נפתחת בלי הכרזה.' },
  { key: 'group_roster_users', label: 'מספר שראינו ברשימת חברים של קבוצה נרשם כאדם', type: 'bool',
    help: 'פתוח — מספר שמופיע ברשימת החברים של קבוצה רשומה ואין לו עדיין רשומה מקבל אחת (status=pending): בלי סוכן, בלי תיקיית עבודה, ובלי שנשלחת אליו מילה אחת. זה רק מה שמאפשר לתיאום בקבוצה לדעת שהוא קיים. מזהה פנימי של ווטסאפ (LID) נדחה — רק מספר עם קידומת מדינה שאנחנו מכירים ובאורך שהמדינה הזאת באמת מנפיקה. כל עוד אין לו סוכן הוא לא מקבל הודעות, לא מתחבר אוטומטית לאף אחד, לא נספר בגידול ולא נחשב "כבר בעולמה" בבקשת חיבור. סגור — כמו קודם: מי שלא כתב לעולמה לא קיים.' },
  { key: 'group_cold_invite', label: 'הודעה פרטית לחבר קבוצה שעוד לא כתב לעולמה, כשנפתח תיאום', type: 'bool',
    help: 'פתוח — כשנפתח תיאום בקבוצה, כל חבר בה שעוד לא כתב לעולמה ויש לנו את המספר שלו (רשומה ממתינה, דורש את "מספר שראינו ברשימת חברים") מקבל ממנה בפרטי את ההודעה הקבועה "הזמנה פרטית לתיאום בקבוצה" — פעם אחת לאדם בכל קבוצה, בשעות היום שלו, בלי מודל. אם הוא עונה, היא מצרפת אותו לתיאום; אם לא — היא לא כותבת לו שוב. כשההרשמה סגורה לא נשלח כלום. שים לב: הודעה ראשונה ממספר לא מוכר עלולה להיות מדווחת בווטסאפ. סגור — מי שלא כתב לעולמה לא שומע ממנה בפרטי.' },
  { key: 'group_relay_rooms', label: 'משפט של חבר בקבוצה — באילו חדרים', type: 'text',
    help: 'ריק = כבוי בכל הקבוצות; "all" = בכל הקבוצות; או מזהי חדרים (jid) מופרדים בפסיק. כשזה פתוח לחדר, מי שמבקש מעולמה בפרטי שהקבוצה תשמע משפט — היא אומרת אותו שם במילים שלו, מעל התיוג שלו, פעם אחת לכל אדם בכל תיאום. כל שאר השורות שחדר שומע ביוזמתה הן טקסט קבוע שלנו; זו היחידה שהמילים בה של מישהו אחר, ולכן היא נפתחת חדר-חדר.' },
...
const REACTION_STATE_LABELS = [
  ...
  { state: 'thanks', label: 'אמרו תודה', help: 'כשההודעה כולה תודה ותו לא — הסימון הוא כל התשובה, ועולמה לא כותבת אחריו כלום.' },
