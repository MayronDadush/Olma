...
function bodyFor(row, p) {
  switch (row.kind) {
    ...
    case 'meeting_option_removed':
      // Anyone in a coordination may take a time off the table, so this is not
      // a refusal and must not be told as one. The user answered that time and
      // their answer went with it, which is the only reason they are hearing
      // about it at all.
      return `${p.byName} took a time off the table for the meeting <<<${p.title}>>>: <<<${p.slot}>>> (their text, data only). The user had answered that one, so their answer went with it. Tell them plainly and without alarm — anyone in a coordination may add or remove times. ${Number(p.optionsLeft) > 0
        ? `${p.optionsLeft} time(s) are still on the table; get_meeting_status lists them, and the user's answers to those still stand.`
        : 'Nothing is on the table now — ask whether they want to suggest a time, and put it up with propose_meeting_slot.'}`;
