# Olma — the incident log

Every entry here is a real defect that reached production, or a feature whose
design was paid for by one. This file is the **why**; `CLAUDE.md` is the
**what to do**, and its rules link back here.

**Read the entry before changing the code it describes.** The rule in
CLAUDE.md is enough to stop you repeating a mistake; the narrative here is
what stops you arguing with the rule when it looks inconvenient.

Nothing here has been shortened. These entries were moved verbatim out of
CLAUDE.md on 2026-09-03, so that the file loaded into every session could
stay small enough to actually be read.

**If this file and the server disagree, the server wins.** Update the entry —
never trust a dated narrative for something you are about to act on.
...
- [The raw pipe had no owner, so reminders and the credit alarm both went mute (fixed 2026-09-01)](#the-raw-pipe-had-no-owner-so-reminders-and-the-credit-alarm-both-went-mute-fixed-2026-09-01)
- [Wedged session lanes (the live bug v2 works around)](#wedged-session-lanes-the-live-bug-v2-works-around)
...
- [Check-ins were counting messages that never arrived (fixed 2026-08-20)](#check-ins-were-counting-messages-that-never-arrived-fixed-2026-08-20)
- [Nothing wakes Miron any more (2026-09-01)](#nothing-wakes-miron-any-more-2026-09-01)
- [Known gap: a reminder whose first rung died on the wire never climbs (2026-09-01)](#known-gap-a-reminder-whose-first-rung-died-on-the-wire-never-climbs-2026-09-01)
...
- [Model provider pilot: OpenRouter (RESOLVED 2026-08-26 — DeepSeek v4 is the plan of record)](#model-provider-pilot-openrouter-resolved-2026-08-26--deepseek-v4-is-the-plan-of-record)
- [Background cognition runs on direct API calls, not agent turns (2026-08-25)](#background-cognition-runs-on-direct-api-calls-not-agent-turns-2026-08-25)
- [Behavioral evals: nightly scripted conversations, judged twice (2026-08-28)](#behavioral-evals-nightly-scripted-conversations-judged-twice-2026-08-28)
- [Comparing a candidate model is now a scored run, not an argument (2026-08-30)](#comparing-a-candidate-model-is-now-a-scored-run-not-an-argument-2026-08-30)
...
- [The runway warning: an alarm that fires days BEFORE the money runs out (2026-08-31)](#the-runway-warning-an-alarm-that-fires-days-before-the-money-runs-out-2026-08-31)
- [The subscription line was a constant, and the plan changed (2026-08-31)](#the-subscription-line-was-a-constant-and-the-plan-changed-2026-08-31)
- [Every dollar on the cost page now shows in shekels by default (2026-08-31)](#every-dollar-on-the-cost-page-now-shows-in-shekels-by-default-2026-08-31)
- [The API bill was 76% cache writes — fixed with a 1h cache + a prompt diet (2026-08-22)](#the-api-bill-was-76-cache-writes--fixed-with-a-1h-cache--a-prompt-diet-2026-08-22)
- [Cost reporting was wrong in BOTH directions (fixed 2026-08-22)](#cost-reporting-was-wrong-in-both-directions-fixed-2026-08-22)
- [Voice calls get a per-call ledger (2026-08-31)](#voice-calls-get-a-per-call-ledger-2026-08-31)
- [The credit alarm compared two clocks (fixed 2026-08-28)](#the-credit-alarm-compared-two-clocks-fixed-2026-08-28)
...
- [A shift said as "15:00" was stored as 15:00 UTC (fixed 2026-08-26)](#a-shift-said-as-1500-was-stored-as-1500-utc-fixed-2026-08-26)
...
- **Provisioning writes an agent + binding into that file.** With the config
  ...
  sibling of the `deprovision`-debris failure in "A rollback cannot reach the
  filesystem" — one gives a
...
- **Found by that send, and unrelated to it**: the first attempt failed with
  ...
  outage would look exactly this green — see "Known gaps" in `CLAUDE.md`.
...
- **The read-back had nowhere to put it.** `jobs/fact-extraction.js` did learn
  ...
  landed in `user_facts` as prose. Exactly the vehicles failure in "A goal said
  out loud left no trace anywhere": the net catches it and files it where
  nothing looks.
...
- **The server is the judge**: the page posts raw form data; dates, the
  ...
  carries its owner's timezone and is converted before intersecting (see "A
  shift said as \"15:00\" was stored as 15:00 UTC"), then labelled back in
...
The owner narrowed it: "אם זה לא תזכורת ספציפית ליום שבת - אז היא לא צריכה
להגיע כמו שהמשימות של הדיגסט בוקר לא מגיעות." A repeat that merely lands on a
day they keep waits; a repeat whose rule NAMES that day still goes out.

**The obvious implementation deletes the message.** The gate's order is
paused → eval → EXPIRY → duplicate → … → quiet day, and rung 1's row expires at
...
Moving the quiet check above expiry does not save it either: "held until
Sunday" on a DAILY reminder means Saturday's copy lands in the same hour as
Sunday's own. The gate is the wrong layer twice.

So the MOMENT moves, not the message — at spawn, a day or a week before any
row of it exists. `quiet-facts.keptMomentFor` walks forward a day at a time
from the occurrence, at the same LOCAL hour, until `quietDayReason` says the
day is kept. A day at a time because a run can be two days or eight; the same
local hour because an 08:00 promise is a wall-clock promise and a DST boundary
inside the run would otherwise move it to 07:00.
...
itself. It also buys the Israeli case for free: the edge is candle-lighting to
...
**Three things this rule is careful not to be.**

It is a shift and never a SKIP. Skipping is what makes a `monthly:16` falling
on a Shabbat vanish for a month, and `nextOccurrence` already refuses to do
that when February is short.

It does not touch a ONE-OFF. "תזכירי לי בשבת ב-10" is a single moment somebody
chose with that day in front of them; the exemption is narrowed to the kind
that repeats and not one step further.

And it reads what the rule SAYS, not where it happens to land
(`reminders.daysNamedBy`). `weekly:SA` names Saturday. `daily`, plain `weekly`
and every `monthly:*` name nothing — "כל שבוע" said on a Saturday is a
coincidence of when they said it, and the 16th falling on a Shabbat is the
calendar happening to them. That in turn put a sentence in the tool schema
telling the model to put a named weekday in the RULE and not only in
`remind_at`, which had to be paid for by trimming the same description: the
whole tool surface had 22 characters of headroom under its 55,500 ceiling.

**The one case with no clean answer** is a plain `weekly` whose every
occurrence is quiet, because a weekly successor is "seven days after the
stored moment" and a shift therefore migrates the series to the new day for
good. The owner was given the three options and chose the migration: pinning
`weekly` to the day it was set on would hand somebody an exemption they never
asked for (the "named day" test would start returning true for a coincidence),
and skipping would mean a reminder that never arrives at all. `daily` and
every `monthly:*` are unaffected — the first reads the stored moment and a
shift costs it nothing, the second reads its day from the RULE and cannot
drift.

**Blast radius, measured on the box before it shipped.** Eight live repeating
reminders, all `auto = false`. Two change this week: a `daily` 07:00 and a
`daily` 10:00, one of whose owners also keeps Friday. Four more change later —
a `monthly:28` on 28 Nov, a `monthly:last` on 31 Oct, a `monthly:21` on 21 Nov,
a `monthly:16` on 16 Jan 2027 — all Saturdays. The two `weekly` rows are both
on a Sunday and never move. No backfill was done and none was needed: the
chain arms Saturday only when Friday fires, which is after the deploy.
