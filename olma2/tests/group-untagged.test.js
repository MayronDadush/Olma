...
test('the plugin registers before_dispatch beside its other three hooks', async () => {
  const hooks = [];
  plugin.default.register({
    pluginConfig: {},
    on: (name) => hooks.push(name),
  });
  assert.deepEqual(hooks, ['before_prompt_build', 'llm_input', 'before_dispatch', 'reply_payload_sending']);
});
