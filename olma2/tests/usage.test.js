...
// v4.1-flash went live pinned to Novita on 2026-09-25; the pilots before that
// ran unpinned at DeepSeek's listing. Re-pricing the pilots at Novita's rate
// would restate two days of history at nearly twice what they cost.
test('v4.1-flash is priced at the pinned provider from the switch, and at the listing before it', () => {
  ...
  assert.equal(after.input, 0.285);
  assert.equal(after.output, 1.14);
  assert.equal(pricing.rateFor('openrouter/deepseek/deepseek-v4.1-flash').rate.input, 0.285, 'no day = today');
});
