...
// when the nightly pass declines to write a task the person already has, it is
// a dashboard row and never a message. Printed beside what the same job DID
// capture, because "stopped 2" means different things over 2 captures and 200.
test('the duplicates the nightly pass stopped are counted on the page, beside what it did capture', async () => {
  ...
  await withTx(db.pool, async (c) => {
    ...
    // night is evidence, and dropping it would make the ratio read high.
  ...
  const html = await section.renderMetrics(db.pool);
  assert.match(html, /כפילויות שנעצרו בלילה/);
