...
// Miron, 2026-09-13 (main, #366): a "full" digest with dozens of open items
// came back as one wall of text, not a picture. `render_schedule_card`
// (domain/schedule-card.js, LIMITS.totalItems) refuses outright past its own
// ceiling, so a turn ordered to draw past it discovers the refusal mid-turn —
...
test('past the card\'s own ceiling the tool hands over the block, and never an order to draw', async () => {
  ...
  assert.doesNotMatch(text, /\b3 or more open items/);
});
